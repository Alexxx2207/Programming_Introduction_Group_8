...
Да се напише калкулатор, който първо приема 2 реални числа `a` и `b` и после приема символ $op \in \{\text{'+'},\ \text{'-'},\ \text{'*'},\ \text{'/'},\ \text{'\%'}\}$
, като принтира резултата от `a` `op` `b`. Операциите са съответни на тези в С++.
